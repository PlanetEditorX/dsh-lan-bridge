...
export function createLanProxy(options = {}) {
  ...
  const server = http.createServer((req, res) => {
    const upstream = http.request({
      ...
      // Never let clients cache the HTML document: the harness sends no
      // cache headers, so mobile browsers heuristically cache a stale
      // pre-polyfill index.html and show empty lists / no onboarding dialog.
      if (isHtml) upstreamRes.headers["cache-control"] = "no-store";
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    upstream.on("upgrade", (upstreamRes, upstreamSocket, upstreamHead) => {
      ...
      try {
        ...
        // NOTE: upstreamHead is server->client data (early WebSocket frames the
        // harness pushed right after the 101). It MUST go to the client socket —
        // writing it back upstream dropped the readiness-handshake frames, so
        // bridged clients connected but never received a single frame (empty
        // session list, no onboarding dialog, forever).
